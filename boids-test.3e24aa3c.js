...
class Rectangle {
    ...
    contains(item) {
        ...
        return px >= this.x - this.w && px < this.x + this.w && py >= this.y - this.h && py < this.y + this.h;
    }
    intersects(range) {
...
class QuadTree {
    ...
    query(range, found = []) {
        if (!this.boundary.intersects(range)) return found; // Empty array if range doesn't intersect this quadrant
        for (let p of this.points)if (range.contains(p)) found.push(p); // Push the actual Boid, not just the Point!
        if (this.divided) {
...
class Predator {
    ...
    hunt(qtree, flock) {
        if (this.target) {
            ...
            if (this.target.isDead || d > this.perceptionRadius || !flock.includes(this.target)) this.target = null;
        }
        if (!this.target) {
        ...
        if (this.target) {
            let desired = new Vector(this.target.position.x, this.target.position.y).sub(this.position);
...
class Boid {
    ...
    flock(localBoids) {
        ...
        if (total > 0) {
            alignment.mult(1 / total).normalize().mult(this.maxSpeed).sub(this.velocity).limit(this.maxForce);
            cohesion.mult(1 / total).sub(this.position).normalize().mult(this.maxSpeed).sub(this.velocity).limit(this.maxForce);
            separation.mult(1 / total).normalize().mult(this.maxSpeed).sub(this.velocity).limit(this.maxForce);
        }
    ...
    update(width, height) {
        ...
        if (this.life < fadeThreshold) this.opacity = Math.max(0, this.life / fadeThreshold);
...
resize();
for(let i = 0; i < 150; i++)flock.push(new Boid(Math.random() * width, Math.random() * height));
...
function animate(timestamp) {
    ...
    for (let boid of flock)if (!boid.isDead) qtree.insert(boid);
    // --- UPDATE PREDATORS ---
    ...
    // --- NEW: DRAW THE TREE ---
    if (params.showTree) qtree.draw(ctx);
    // ---------------------------------
    ...
    if (mouse.isLeftDown) {
        ...
        while(flock.length > 2000)flock.shift();
    }
    ...
    for(let i = flock.length - 1; i >= 0; i--)if (flock[i].isDead) {
...
function setupUI() {
    ...
    if (trailCheckbox) trailCheckbox.addEventListener("change", (e)=>{
        params.trails = e.target.checked;
    });
    const treeCheckbox = document.getElementById("tree-checkbox");
    if (treeCheckbox) treeCheckbox.addEventListener("change", (e)=>{
    ...
    resetBtn.addEventListener("click", ()=>{
        ...
        bindings.forEach((bind)=>{
            ...
            if (slider && valueDisplay) {
                ...
                valueDisplay.innerText = params[bind.key].toFixed(bind.id === "rad" ? 0 : 1); // Update the text
            }
    ...
    if (predatorBtn) predatorBtn.addEventListener("click", ()=>{
...
//# sourceMappingURL=boids-test.3e24aa3c.js.map
